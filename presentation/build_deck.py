...
def agenda_slide(prs, page):
    ...
    items = [
        ...
        ("06", "분석: 분해 · 예측 · 모델 비교", "추세/요일 분리, 14일 외삽, SARIMA 비교"),
        ("07", "결론: 트렌딩 태그 + 우리 계정", "지금 뜨는 태그 · 가상 계정 도달 분해/예측"),
...
def build():
    ...
    chart_slide(
        prs, next(pg), "모델 비교 — 베이스라인 vs SARIMA", "06 · Model comparison", "09_sarima_compare.png",
        [
    ...
    chart_slide(
        prs, next(pg), "되돌아온 질문: 우리 계정은 어떻게 될까", "07 · Our account", "10_account_decomp.png",
        [
    ...
    chart_slide(
        prs, next(pg), "우리 계정 14일 예측", "07 · Our account", "11_account_forecast.png",
        [
